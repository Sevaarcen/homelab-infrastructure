...
ANSIBLE_STDOUT_CALLBACK=json ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook --private-key "$private_key" -e "pub_key='$public_key' node_token='$node_token' k3s_url='$k3s_loadbalancer_url'" -u "$ansible_user" -i "$hosts" ./ansible-playbooks/k3s-worker-setup.yaml |& tee ./logs/k3s-worker-ansible-deployment.log | jq '[leaf_paths as $path | {"key": $path | join("."), "value": getpath($path) | tostring}] | from_entries'